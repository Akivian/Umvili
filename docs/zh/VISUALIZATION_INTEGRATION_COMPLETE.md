...
### 常规图表（使用MultiLineChart，单线显示）

1. **Population（人口）**
   - 显示智能体总数变化
   - 颜色：蓝色 (CHART_LINE_1)

2. **Average Sugar（平均糖量）**
   - 显示智能体平均糖量变化
   - 颜色：橙色 (CHART_LINE_2)
...
1. **`_initialize_charts()`**
   - 改为使用`MultiLineChart`
   - 保持单线显示（不显示图例）

2. **`_initialize_training_charts()`**（新增）
   - 初始化4个训练指标图表
   - 预添加IQL和QMIX曲线

3. **`_update_charts()`**
   - 更新为使用`add_data_point_conditional()`
   - 支持可选的`simulation_data`参数

4. **`_update_training_charts()`**（新增）

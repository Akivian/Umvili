...
- ✅ **多资源类型系统**：
  ...
  - **Spice（香料）**：高价值稀缺资源，在 1–2 个随机中心点周围极小范围内集中生成，消耗殆尽后在新的“安全区域”随机重新生成
  - **Hazard（危险区）**：深血红色危险区域，从单一随机核心点开始，核心区域随时间缓慢扩张，最终形成一大片连通危险区
- ✅ **动态资源系统**：
  - Spice 动态生成：在 1–2 个随机中心点周围极小范围内生成，消耗殆尽后延迟约 80 步在新的位置重新生成
  - Hazard 动态扩张：以核心点为中心持续向外扩张，在覆盖约 1/3 地图面积后通过“边缘轻微扩张/收缩”的方式保持类似呼吸的稳定形状
  - 资源竞争机制：Spice 奖励倍率 5.0，鼓励智能体在稀缺高价值区域中竞争
- ✅ **Hazard 危险机制**：
  - 危险区内资源完全清空且不再生成
  - 误入危险区的智能体每步损失大量 sugar（当前实现中 `hazard_damage_per_step = 6.0`，且只对有效危险区生效）
  - 危险区在长时间运行后稳定覆盖约 1/3 地图，比 spice 区大得多，形成明显且持久的危险地带
  - 深血红色可视化，并与 Sugar/Spice 叠加顺序严格控制，营造直观的风险反馈
- ✅ **多资源可视化**：
  - Sugar：绿色渐变背景
  - Spice：金黄色半透明叠加（约 70% 不透明度）
  - Hazard：深血红色半透明叠加（约 78% 不透明度）
  - 清晰的绘制顺序和层次，避免视觉混乱
...
**最后更新**：2025年12月4日  
**文档版本**：3.1.0  
**项目状态**：Phase 1–6 已完成，Phase 7 核心功能已落地（Q 值热图、网络状态可视化），Phase 8 正在实现交互式配置基础设施，Phase 9–10 仍处于规划阶段
